...
vector<string> parse::parseFile(char* filepath){
  vector<string> out;
  string curr_read = "";
...
vector<string> parse::parseFile(char* filepath, char delim){
  vector<string> out;
...
vector<string> parse::split(const string read, char delim){
  ...
  for(string::const_iterator it = read.begin(); it != read.end(); it++){
    if(*it == delim){
